...
public class Variable<T>
{
    ...
    public Variable(string name, T value, int scope, int line)
    {
        this.name = name;
        this.value = value;
        this.scope = scope;
...
public class CodePanelController : MonoBehaviour
{
        ...
        if (selectedBlock != null)
        {
            ...
            else
            {
                ...
                if (selectedBlock.type == BlockType.DECLARE_INT ||
                    ...
                    selectedBlock.type == BlockType.FOR)
                {
    ...
    public bool tryInsertVariable(BlockType type, string name, VariableGap gap)
    {
        ...
        {
            case ValueType.INT:
                linedec = int_variables[name].lineDeclared;
                scopevar = int_variables[name].scope;
                break;
            case ValueType.STRING:
                linedec = string_variables[name].lineDeclared;
                scopevar = string_variables[name].scope;
                break;
            case ValueType.FLOAT:
                linedec = float_variables[name].lineDeclared;
            ...
            (linedec < 1 || isInSameScope(parent.id, siblingIndex, parent.type)))
        {
            ...
            bg.insertVariable(gap, block.GetComponent<Block>(), name, valType);
            return true;
